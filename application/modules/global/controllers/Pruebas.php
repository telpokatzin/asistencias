...
class Pruebas extends IS_Controller {
	...
	public function email() {
		$data = array(
			 ...
			 'message' 	=> $this->parser_view("mail/prueba", $this->session->userdata())
			,'titulo' 	=> 'Pruebas'
			,'view' 	=> 'main-template'
			,'asunto' 	=> 'Pruebas'
			,'para' 	=> 'saul.hernandez@isolution.mx'
		);

		// Send email
		$resultado = modules::run('global/Correos/send_mail', $data);
	...
	public function pdf() {
		$settings = array(
			...
			,'content_file' => 'Archivo generado desde el sistema de <b>'.get_var('app_title').'</b>: '. date('Y-m-d H:i:s')
			,'load_file' 	=> TRUE
		);
