...
class IS_Rest extends IS_Controller {
	...
	private function _prepare_basic_auth()
	{
		...
		{
			if($this->vars->cfg['rest_md5_onoff']){
			$username = ($this->input->server('PHP_AUTH_USER')===$this->vars->cfg['rest_user_md5'])?$this->vars->cfg['rest_user']:'';
			$password = ($this->input->server('PHP_AUTH_PW')===$this->vars->cfg['rest_pass_md5'])?$this->vars->cfg['rest_pass']:'';
			}else{
